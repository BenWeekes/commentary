...
class TTSEngine:
            ...
            if self.on_telemetry:
                try:
                    self.on_telemetry({
                        ...
                        "translation_context_enabled": meta.get("translation_context_enabled"),
                        "translation_context_used": meta.get("translation_context_used"),
                        "previous_source": meta.get("previous_source"),
                        "previous_translation": meta.get("previous_translation"),
                        "previous_audio_start": meta.get("previous_audio_start"),
                        "previous_audio_end": meta.get("previous_audio_end"),
                        "previous_speaker": meta.get("previous_speaker"),
                        "previous_gap_s": meta.get("previous_gap_s"),
                        "split_group_id": meta.get("split_group_id"),
                        "split_part_index": meta.get("split_part_index"),
    ...
    def _process_item(self, item, lang_version=None):
        ...
        base_result = {
            ...
            **(item_meta or {}),
            ...
            "translation_context_enabled": (translation_meta or {}).get("translation_context_enabled") if isinstance(translation_meta, dict) else None,
            "translation_context_used": (translation_meta or {}).get("translation_context_used") if isinstance(translation_meta, dict) else None,
            "previous_source": (translation_meta or {}).get("previous_source") if isinstance(translation_meta, dict) else None,
            "previous_translation": (translation_meta or {}).get("previous_translation") if isinstance(translation_meta, dict) else None,
            "previous_audio_start": (translation_meta or {}).get("previous_audio_start") if isinstance(translation_meta, dict) else None,
            "previous_audio_end": (translation_meta or {}).get("previous_audio_end") if isinstance(translation_meta, dict) else None,
            "previous_speaker": (translation_meta or {}).get("previous_speaker") if isinstance(translation_meta, dict) else None,
            "previous_gap_s": (translation_meta or {}).get("previous_gap_s") if isinstance(translation_meta, dict) else None,
    ...
    def _emit_dropped_result(self, result, status="dropped", interrupted_by=""):
        ...
        self._skipped_meta.append({
            ...
            "translation_context_enabled": result.get("translation_context_enabled"),
            "translation_context_used": result.get("translation_context_used"),
            "previous_source": result.get("previous_source"),
            "previous_translation": result.get("previous_translation"),
            "previous_audio_start": result.get("previous_audio_start"),
            "previous_audio_end": result.get("previous_audio_end"),
            "previous_speaker": result.get("previous_speaker"),
            "previous_gap_s": result.get("previous_gap_s"),
