...
Same pipeline as v5 (single gpt-5.4 vision call per burst, no Gemini, no
...
on gpt-5.4? If yes, hybrid isn't adding value.
...
OUT_JSONL = BASE / 'commentary_v11.jsonl'
OUT_KEPT = BASE / 'commentary_v11_kept.txt'
OUT_SCHED = BASE / 'commentary_v11_scheduled.jsonl'
...
def main():
    ...
    print(f"v11: single {MODEL} + playerist + rich context (no hybrid)")
    ...
    with open(OUT_KEPT, 'w') as f:
        f.write(f"# v11: single {MODEL} + playerist + rich context (control)\n")
