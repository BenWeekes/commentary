#!/usr/bin/env python3
"""v8 arbiter/vision variants for cutting hallucinations.

Usage:  python run_v8_variants.py <variant>

  a — tightened arbiter rubric, same models (mini vision, mini arbiter+frame)
  b — gpt-5.5 arbiter with frame; same vision + same rubric as base v8
  c — gpt-5.5 arbiter + tightened rubric (a + b combined)
  d — confidence-gated vision: models produce SPECIFIC only when confidence=high
      (base arbiter, same as v8 baseline)

All variants share v8's rich context, dual vision, arbiter-in-loop, FR translate.
"""
...
assert VARIANT in ('a', 'b', 'c', 'd'), f"variant must be a/b/c/d, got {VARIANT}"

BASE = Path('/home/ubuntu/commentary/experiments/ai_commentator')
FRAMES_DIR = BASE / 'frames'
OUT_JSONL = BASE / f'commentary_v8{VARIANT}.jsonl'
OUT_KEPT = BASE / f'commentary_v8{VARIANT}_kept.txt'
OUT_SCHED = BASE / f'commentary_v8{VARIANT}_scheduled.jsonl'

OAI_MODEL = 'gpt-5.4-mini'
GEMINI_MODEL = 'gemini-2.5-flash'
# Variant b/c use gpt-5.5 as arbiter; a/d use gpt-5.4-mini
ARBITER_MODEL = 'gpt-5.5' if VARIANT in ('b', 'c') else 'gpt-5.4-mini'
# Variant a/c use tightened arbiter rubric
TIGHTENED_ARBITER = VARIANT in ('a', 'c')
# Variant d uses confidence-gated vision
CONFIDENCE_GATED_VISION = VARIANT == 'd'
...
VISION_PROMPT_BASE = """You are a live English football commentator on a Bundesliga broadcast.
...
Produce TWO variants + a confidence level, as JSON:
  ...
  "safe"     — 5-10 words. Generic but INFORMATIVE description. Who has the
               ball, where, and roughly what phase of play. NO specific
               outcomes ("saves!", "buries it!"). Vary phrasing — do NOT just
               say "X on the ball" every time.
...
VISION_CONFIDENCE_GATED_TAIL = """
CONFIDENCE GATE — IMPORTANT for this variant:
- Only fill "specific" if you are HIGH confidence in the subject AND action.
- Medium/low confidence → set "specific" to null, only fill "safe".
- "safe" MUST NOT claim events (subs, cards, saves, tackles, shots) unless
  the event is unambiguously visible in the newest frame.
- When in doubt, prefer more generic descriptions of ball location + team +
  tempo. NO_CALL is fine when the frame is a replay, crowd shot, or static.
"""

VISION_PROMPT_TAIL = """
If nothing is worth commenting on (routine possession, replay, static, crowd),
output:
...
- Rotate team-name aliases; same short name twice in a row is repetitive.
- GENERIC OVER INCORRECT.
"""


ARBITER_PROMPT_BASE = """You are the ARBITER + JUDGE for a live football
commentary AI. You get:
  - The newest video frame the commentators just spoke about
  - Two candidate outputs from two vision LLMs, each with a "specific"
    (detailed) and "safe" (generic) + confidence
  - The running state (recent accepted lines, sub history)

Your job:
  1. Pick BEST candidate to speak, or NO_CALL. Verbatim pick — no rewriting.
       "A_specific" | "A_safe" | "B_specific" | "B_safe" | "NO_CALL"
  2. Verify the frame — REJECT specifics that claim events not visibly
     happening. Prefer safer variant when a specific is speculative.
  3. Score the picked line 1-5.
  4. Translate the picked line into natural French sports commentary.
  5. One-sentence reason.

Return JSON ONLY:
  {"choice":"...", "en":"<verbatim>", "fr":"<translation>", "quality":0-5, "reason":"<sentence>"}
"""

ARBITER_TIGHTENED_TAIL = """
STRICT FRAME-GROUNDING RUBRIC (this variant is intentionally strict):
- Reject any claim about state that is NOT visible in the frame RIGHT NOW.
  Examples of NOT-visible-in-a-single-frame:
    * "injured" (only "down" is visible)
    * "on a booking" (a yellow card earlier is not in THIS frame)
    * "key creator" / narrative adjectives (context, not visible)
    * kit-colour descriptions unless the colour is clearly what you'd expect
    * "just sent ball out" (only a static ball is visible)
    * "substitute coming on" (unless the 4th-official board with numbers IS the frame)
    * "applauds the crowd" (only "hand raised" is visible)
- Score ≤ 3 for any line that includes an unverifiable claim.
- Score ≤ 2 for lines with SUBSTITUTION / GOAL / CARD claims when the frame
  doesn't show that event.
...
DEFAULT: err on the side of NO_CALL. Silence is better than a wrong claim.
"""
...
def build_vision_prompt(rich_ctx, latest_time_s, previous_calls, alias_usage,
    ...
    if CONFIDENCE_GATED_VISION:
        prompt += VISION_CONFIDENCE_GATED_TAIL
    prompt += VISION_PROMPT_TAIL
    return f"""{prompt}
...
def build_arbiter_prompt(a_json, b_json, previous_calls, sub_hist, pitch_state,
    ...
    previous = "\n".join(f"  - {c}" for c in previous_calls[-12:]) or "  - none"
    prompt = ARBITER_PROMPT_BASE
    if TIGHTENED_ARBITER:
        prompt += ARBITER_TIGHTENED_TAIL
    prompt += ARBITER_PROMPT_TAIL
    return f"""{prompt}
...
def arbiter_call(a_json, b_json, previous, sub_hist, pitch_state,
    ...
    if ARBITER_MODEL.startswith('gpt-5.5'):
        # gpt-5.5 wants reasoning effort not temperature
...
def main():
    print(f"VARIANT={VARIANT}  arbiter={ARBITER_MODEL}  tight_rubric={TIGHTENED_ARBITER}  conf_gate_vision={CONFIDENCE_GATED_VISION}")
    ...
    for burst_idx, (i, latest_time_s, burst, newest) in enumerate(bursts):
        ...
        a_safe   = bool(a_json and a_json.get('safe'))
        b_safe   = bool(b_json and b_json.get('safe'))
        # In confidence-gated mode, only "safe" fills would still count as speaking
        arbiter_json = None; arb_ms = 0
        chosen = None; choice = 'NO_CALL'
        fr_text = ''; quality = 0; reason = ''
        if a_speaks or b_speaks or a_safe or b_safe:
        ...
        if time.time() - last_print > 15:
            print(f"  b{burst_idx}/{len(bursts)} t={latest_time_s:.1f}s acc={len(accepted)} nc={no_call} rep={repetitive} el={time.time()-t_start:.0f}s ch={choice:<10} last={(chosen or '')[:55]!r}")
            last_print = time.time()

    print(f"\nv8{VARIANT} Summary: attempts={len(all_attempts)} accepted={len(accepted)} skipped={skipped} no_call={no_call} rep={repetitive} err={errors}")
    ...
    with open(OUT_KEPT, 'w') as f:
        f.write(f"# v8{VARIANT}: arbiter={ARBITER_MODEL} tight={TIGHTENED_ARBITER} confgate={CONFIDENCE_GATED_VISION}\n")
