...
func main() {
	...
	*bStop = false
	...
	argus := os.Args
	if len(argus) < 3 {
		fmt.Println("usage: send_h264_uid73 <app_id> <channel> [h264_file]")
		return
	}
	appid := argus[1]
	channelName := argus[2]
	videoFile := "/tmp/agora_test_data/extracted/test_data/send_video.h264"
	if len(argus) >= 4 {
		videoFile = argus[3]
	}
	...
	if cert != "" {
		...
		var err error
		token, err = rtctokenbuilder.BuildTokenWithUid(appid, cert, channelName, 73,
	...
	for !*bStop {
		...
		if ret != 0 {
			fmt.Printf("PushVideoEncodedData ret=%d frameType=%d size=%d\n", ret, frameType, len(data))
		}
