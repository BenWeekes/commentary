...
| `translation_context_enabled` | bool | `false` | Pass previous source-time utterance context into translation prompts |
...
- `translation_context_used`, `previous_source`, `previous_translation`, `previous_gap_s` — previous-utterance context diagnostics. Context is selected by source audio time, not completion wall-clock, so parallel prepare cannot feed a future utterance as prior context.
...
- `original_play_at`, `split_chain_gap_ms`, `split_chain_advance_ms` — present when a forced-split continuation was pulled earlier to play directly after the previous split part. Forced-split chaining requires immediate split adjacency and is capped at 4000ms to avoid pathological stale joins.
- `continuity_chain_source_gap_ms`, `continuity_chain_gap_ms`, `continuity_chain_advance_ms` — present when a normal same-speaker STT pair with a short source gap was pulled closer together. This is separate from forced-split chaining, never uses grammar heuristics, and is capped to avoid large timing changes.
